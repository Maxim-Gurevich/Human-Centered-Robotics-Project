...
import signal
import shutil
from pnc.wbc.manager.hand_trajectory_manager import HandTrajectoryManager as htm
...
if __name__ == "__main__":
    ...
    while (1):
        ...
        htm.set_object_pos(sensor_data_o[0])
        graphtime.append(count/100)
        graph_object_pos.append(sensor_data_o[0][2])
        
        if count == -100: 
            fig, axs = plt.subplots(2,1)
            axs[0].plot(graphtime, graph_object_pos)
            axs[0].set_ylabel('Position (m)')
            axs[0].legend(['x', 'y'])
            axs[0].grid(True)
            
            axs[1].plot(graphtime, graph_object_pos)
