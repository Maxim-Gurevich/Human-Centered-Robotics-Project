...
class FootTrajectoryManager(object):
    ...
    def use_current(self):
        ...
        """
        foot_pos_des = self._robot.get_link_iso(self._target_id)[0:3,3]
        foot_lin_vel_des = self._robot.get_link_vel(self._target_id)[3:6]
        self._pos_task.update_desired(foot_pos_des, foot_lin_vel_des,
                                      np.zeros(3))

        foot_rot_des = util.rot_to_quat(self._robot.get_link_iso(self._target_id)[0:3,0:3])
        foot_ang_vel_des = self._robot.get_link_vel(self._target_id)[0:3]
    ...
    def update_swing_foot_desired(self, curr_time):
        ...
        """
        self.s=(curr_time-self._swing_start_time)/self._swing_duration
        
        if self.s<=.5:
            foot_pos_des = self._pos_traj_init_to_mid.evaluate(self.s*2)
            foot_lin_vel_des = self._pos_traj_init_to_mid.evaluate_first_derivative(self.s*2)
            foot_lin_acc_des = self._pos_traj_init_to_mid.evaluate_second_derivative(self.s*2)

        else:
            foot_pos_des = self._pos_traj_mid_to_end.evaluate((self.s-.5)*2)
            foot_lin_vel_des = self._pos_traj_mid_to_end.evaluate_first_derivative((self.s-.5)*2)
            foot_lin_acc_des = self._pos_traj_mid_to_end.evaluate_second_derivative((self.s-.5)*2)

        foot_quat_des = self._quat_hermite_curve.evaluate(self.s)
        foot_ang_vel_des = self._quat_hermite_curve.evaluate_ang_vel(self.s)
        foot_ang_acc_des = self._quat_hermite_curve.evaluate_ang_acc(self.s)
