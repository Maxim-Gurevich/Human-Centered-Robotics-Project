...
class DCMPlanner(object):
    ...
    def _compute_dcm_ini_ds(self, step_idx, t_ds_ini):#double support phase
        if step_idx == 0:
            return self._ini_dcm_pos
        ## TODO
        dcm_ini_ds = np.zeros(3)
        #############################################################################
        #dcm_ini_ds = np.array(np.exp((-t_ds_ini+self._dcm_ini_list[step_idx])/self._b)\
        #+self._vrp_list[step_idx-1])
        #dcm_ini_ds = np.array(-np.exp(-t_ds_ini/self._b)*(self._vrp_list[step_idx-1]-self._dcm_ini_list[step_idx])+self._vrp_list[step_idx-1])
        r=self._vrp_list[step_idx-1]
        t=-t_ds_ini
        b=self._b
        e0=self._dcm_ini_list[step_idx]
        dcm_ini_ds = np.array(r-np.exp(t/b)*(r-e0))
        #############################################################################
        return dcm_ini_ds

    def _compute_dcm_vel_ini_ds(self, step_idx, t_ds_ini):#double support phase
        if step_idx == 0:
        ...
        e0=self._dcm_ini_list[step_idx]
        dcm_ini_ds = np.array(r-np.exp(t/b)*(r-e0))
        dcm_vel_ini_ds = np.array((dcm_ini_ds-self._vrp_list[step_idx-1])/self._b)
        #############################################################################
        return dcm_vel_ini_ds

    def _compute_dcm_acc_ini_ds(self, step_idx, t_ds_ini):#double support phase
        if step_idx == 0:
            return np.zeros(3)
        ## TODO
        dcm_acc_ini_des = np.zeros(3)
        #############################################################################
        #############################################################################
        return dcm_acc_ini_des

    def _compute_dcm_end_ds(self, step_idx, t_ds_end):#double support phase
        if step_idx == len(self._vrp_list) - 1:
            ...
            #############################################################################
            #dcm_end_ds = np.array(np.exp((-t_ds_end+self._dcm_ini_list[step_idx])/self._b)\
            #+self._vrp_list[step_idx])
            #dcm_end_ds = np.array(-np.exp(-t_ds_end/self._b)*(self._vrp_list[step_idx]-self._dcm_ini_list[step_idx])+self._vrp_list[step_idx])
            r=self._vrp_list[step_idx]
            t=t_ds_end
            b=self._b
            e0=self._dcm_ini_list[step_idx]
            dcm_end_ds = np.array(r-np.exp(t/b)*(r-e0))
           #############################################################################
            return dcm_end_ds

    def _compute_dcm_vel_end_ds(self, step_idx, t_ds_end):#double support phase
        if step_idx == len(self._vrp_list) - 1:
            return np.zeros(3)
        elif step_idx == 0:
            ...
            e0=self._dcm_ini_list[step_idx]
            dcm_end_ds = np.array(r-np.exp(t/b)*(r-e0))
            dcm_vel_end_ds = np.array((dcm_end_ds-self._vrp_list[step_idx])/self._b)
            #############################################################################
            return dcm_vel_end_ds

    def _compute_dcm_acc_end_ds(self, step_idx, t_ds_end):#double support phase
        if step_idx == len(self._vrp_list) - 1:
            return np.zeros(3)
        elif step_idx == 0:
    ...
    def _compute_dcm_ini(self, vrp_d_i, t_step, dcm_eos_i):#initial desired DCMs
        ## TODO
        dcm_ini = np.zeros(3)
        #############################################################################
        #dcm_ini = np.array(np.exp((-t_step+dcm_eos_i)/self._b)+vrp_d_i)
        r=vrp_d_i
        t=-t_step
        b=self._b
        e0=dcm_eos_i
        dcm_ini = np.array(r-np.exp(t/b)*(r-e0))
        #############################################################################
        return dcm_ini
