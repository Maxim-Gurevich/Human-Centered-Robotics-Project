...
class SimConfig(object):
    ...
    INITIAL_POS_WORLD_TO_BASEJOINT = [0, 1, 1.5 - 0.761]
    INITIAL_QUAT_WORLD_TO_BASEJOINT = [0., 0., 1., -1.]
...
class WBCConfig(object):
    ...
    KP_PELVIS = np.array([300., 300., 300])
    KD_PELVIS = np.array([10., 10., 10.])

    KP_UPPER_BODY = 200.
    KD_UPPER_BODY = 10.

    KP_HAND_POS = np.array([200., 200., 200.])
    KD_HAND_POS = np.array([10., 10., 10.])
...
class WalkingConfig(object):
    ...
    COM_HEIGHT = 1.0#1.02  # m
    SWING_HEIGHT = 0.025#0.05  # m

    T_ADDITIONAL_INI_TRANS = 0.  # sec
    T_CONTACT_TRANS = 1#0.45
    T_SWING = 0.5#0.75
    PERCENTAGE_SETTLE = .95#0.9
    ALPHA_DS = 0.5

    NOMINAL_FOOTWIDTH = 0.25#0.25
    NOMINAL_FORWARD_STEP = 0.1 #0.15
    NOMINAL_BACKWARD_STEP = -0.1 #-.15
    NOMINAL_TURN_RADIANS = np.pi / 6
    NOMINAL_STRAFE_DISTANCE = 0.1 #0.1
    
    T_TRANS_DURATION = 1
    T_REACHING_DURATION = 3
    
    RH_TARGET_POS = np.array([-0.5, 0.55, 1.2])
    LH_TARGET_POS = np.array([0.5, 0.55, 1.2])
    
    RH_TARGET_POSB = np.array([0.5, -0.55, 1.2])
    LH_TARGET_POSB = np.array([-0.5, -0.55, 1.2])
